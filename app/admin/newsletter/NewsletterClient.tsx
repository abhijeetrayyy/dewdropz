...
import { useSkipMount } from '@/components/admin/useSkipMount'
...
export default function NewsletterClient({ initial }: { initial: { rows: NewsletterSubscriber[]; total: number } }) {
  const skipMount = useSkipMount()
  const [subscribers, setSubscribers] = useState<NewsletterSubscriber[]>(initial.rows)
  ...
  // First page came from the server; skip only that fetch.
  useEffect(() => { if (skipMount()) return; load() }, [debouncedSearch, page])
