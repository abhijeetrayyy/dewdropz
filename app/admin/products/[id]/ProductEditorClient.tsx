...
/** Server payload → the shape the form edits. Called twice on mount (once for
 *  the working copy, once for the baseline) so the two can never start out
 *  sharing a reference and comparing equal forever. */
function toForm(data: ProductEditorData): Form {
...
export default function ProductEditorClient({
  ...
  // Everything below is seeded from the server render. There is no fetch on
  // mount, because there is nothing left to fetch: the data came down with the
  // page. This matters more than the query cost — the functions run in US East
  // and the people using this are in India, so a fetch-after-mount is a second
  // Atlantic crossing that cannot start until the first one has finished.
  const [product, setProduct] = useState<Product>(data.product as Product)
  const [allCategories, setAllCategories] = useState<CategoryWithChildren[]>(data.categories ?? [])
  const [allTags, setAllTags] = useState<Tag[]>(data.tags ?? [])
  ...
  const [initial, setInitial] = useState<Form>(() => toForm(data))
  ...
  // No loading branch: this component is only rendered once the server has the
  // data, and the skeleton people actually see now lives in loading.tsx, which
  // Next shows during the navigation itself rather than after it.
