...
export default function ReviewsClient({ initial }: { initial: { rows: Array<Record<string, unknown>>; total: number } }) {
  const skipMount = useSkipMount()
  const { confirm, dialog } = useConfirm()
  const [reviews, setReviews] = useState<Array<Record<string, unknown>>>(initial.rows)
  ...
  useEffect(() => { if (skipMount()) return; load() }, [page])
